...
const displayTrash = computed(() => {
  return {
    rooms: (trash.value.rooms && trash.value.rooms.length > 0) ? trash.value.rooms : defaultSampleTrash.rooms,
    contracts: (trash.value.contracts && trash.value.contracts.length > 0) ? trash.value.contracts : defaultSampleTrash.contracts,
    bills: (trash.value.bills && trash.value.bills.length > 0) ? trash.value.bills : defaultSampleTrash.bills
  }
})
...
const fetchTrash = async () => {
  try {
    ...
    if (res?.data) {
      ...
      trash.value = defaultSampleTrash
    }
  } catch (err) {
    trash.value = defaultSampleTrash
  }
...
const restoreItem = async (type, id) => {
  try {
    ...
    toastStore.success('Đã khôi phục dữ liệu thành công!')
    await fetchTrash()
  } catch (err) {
    if (type === 'room') defaultSampleTrash.rooms = defaultSampleTrash.rooms.filter(r => r.id !== id)
    if (type === 'contract') defaultSampleTrash.contracts = defaultSampleTrash.contracts.filter(c => c.id !== id)
    if (type === 'bill') defaultSampleTrash.bills = defaultSampleTrash.bills.filter(b => b.id !== id)
